...
class MemeRadarApp:
    ...
    async def handle_manifest(self, request):
        manifest_data = {
            "name": "Axiom Meme Radar Pro",
            "short_name": "MemeRadar",
            "start_url": "/",
            "display": "standalone",
    ...
    async def scanner_background_loop(self):
        log_event("Axiom Radar Engine activated. Polling 24/7...", "INFO")
        
        await self.telegram.send_system_message(
            "🔴 *Axiom Solana Radar is ONLINE!*\n"
            "Monitoring DexScreener, Pump.fun, Axiom & X. Real-time alerts will appear below."
        )
            ...
            while True:
                try:
                    ...
                    for token in new_tokens:
                        ...
                        log_event(f"Viral Token: ${token['symbol']} (Liq: ${token['liquidity_usd']:,.0f})", "ALERT")
                        
                        await self.telegram.send_token_alert(token, session=session)
                        await asyncio.sleep(1.5)

                except Exception as e:
                    logger.error(f"Scanner iteration exception: {e}")
                    log_event(f"Scanner warning: {str(e)}", "WARN")
    ...
    async def start_server(self):
        ...
        await site.start()
        log_event(f"Axiom Radar Dashboard running on port {settings.PORT}", "INFO")
